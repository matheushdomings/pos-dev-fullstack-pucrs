...
tabela_DP = sa.Table(oc.dp.__tablename__, metadata, autoload_with=engine) #na classe que representa a tabela de ocorrências
...
#Variável que representa a tabela que se deseja inserir os dados e, um metadados.
tabela_respDP = sa.Table(oc.responsaveldp.__tablename__, metadata, autoload_with=engine) #na classe que representa a tabela de ocorrências

#Inserindo dados a partir de uma conexão com a engrenagem de BD
try:
    sessao.execute(tabela_respDP.insert(), DadosRespoDP)
    sessao.commit()
except ValueError:
    print(ValueError())
...
#Variável que representa a tabela que se deseja inserir os dados e, um metadados.
tabela_municipio = sa.Table(oc.municipio.__tablename__, metadata, autoload_with=engine) #na classe que representa a tabela de ocorrências
...
#Variável que representa a tabela que se deseja inserir os dados e, um metadados.
tabela_ocorrencias = sa.Table(oc.ocorrencia.__tablename__, metadata, autoload_with=engine) #na classe que representa a tabela de ocorrências

#Inserindo dados a partir de uma conexão com a engrenagem de BD
try:
    sessao.execute(tabela_ocorrencias.insert(), DadosOcorrencias)
    sessao.commit()
except ValueError:
    print(ValueError())
